...
var RowRenderer = Class.extend({
	...
	rowFooterTable: function(rowType, row) {
		var renderCell = this.getHtmlRenderer('cell', rowType);
		var rowCellHtml = '';
		var col;
		var cell;
...
//var date = cell.start;
		...
		for (col = 0; col < this.colCnt; col++) {
			cell = this.getCell(row, col);
			//rowCellHtml += renderCell(cell);
			//rowCellHtml += headCellHtml2(cell);
			var date = cell.start;
			rowCellHtml +='<th class="fc-footer  fc-' + dayIDs[date.day()] + '">' +
			'</th>'
		}
		//rowCellHtml = this.bookendCells(rowCellHtml, rowType, row); // apply intro and outro
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	// TODO: move to another class. not applicable to all Grids
	headHtml: function() {
		return '' +this.rowHtml('head') ;
			
	},
	headFooterTable: function() {
		return '' +this.rowFooterTable('head') ;
	...
	headCellHtml: function(cell) {
		...
		return '' +
			'<th class="col-md-1">' +
				'<span>'+htmlEscape(date.format('ddd')) +' '+date.format('DD  MMM') + '</span>'+
			'</th>';
	},
	headCellHtml2: function(cell) {
		var view = this.view;
		var date = cell.start;
		return '' +
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		titleHtml =
			'<span class="fc-title">' +
				event.hours+ // we always want one line of height
			'</span>';
			...
			<table class="table table-striped">\
				...
				<tr>\
					<th class="col-md-3"></th>\
						'+this.dayGrid.headFooterTable() +'\
						<th class="col-md-1 totalHoursWeek"></th>\
				</tr>\
				</tfoot>\
				<tbody></tbody>\
			</table>');
            ...
            while (currentDayStart.isBefore(periodEnd)) {
                ...
                for (var i = eventsCopy.length - 1; i >= 0; --i) {
                    ...
                    if (currentDayStart.isAfter(eventEnd) || (currentDayStart.isSame(eventEnd) && !eventStart.isSame(eventEnd)) || periodEnd.isBefore(eventStart)) {
                        ...
                        var segEl = $('\
							<tr>\
                        		<td  class="col-md-3"><input type="checkbox" id="timesheetsCheckbox" value="'+e+'">&nbsp;<small>['+e.project.id+']</small>&nbsp;<strong>'+e.project.projectName+'</strong><br \><span>'+e.activity.activityName+'<br \><span>'+'</span></td>\
								<td class="col-md-1 " ><span class="fc-sun">0</span></td>\
								<td class="col-md-1 fc-mon" ><span  class="fc-mon">0</span></td>\
								<td class="col-md-1  fc-tue" ><span  class="fc-tue" >0</span></td>\
								<td class="col-md-1  fc-wed" ><span  class="fc-wed">0</span></td>\
								<td class="col-md-1  fc-thu" ><span  class="fc-thu">0</span></td>\
								<td class="col-md-1  fc-fri" ><span  class="fc-fri">0</span></td>\
								<td class="col-md-1 fc-sat" ><span  class="fc-sat">0</span></td>\
								<td class="col-md-1">\
                                <a  style="cursor: pointer;"><span id ="edit" class="glyphicon glyphicon-pencil"></span></a>\
                                <a class="trash" style="cursor: pointer;"><span id="trash" class="glyphicon glyphicon-trash"></span></a>\
                            </td>\
							</tr>');
							var currentDay = '.fc-'+currentDayStart.format('ddd').toLowerCase();
							segEl.find(''+currentDay+'').html(e.hours);
                        tbody.append(segEl);
                        ...
                        (function (_this, myEvent, mySegEl) { //temp bug fix because 'e' seems to change
                            ...
                            segEl.on('mouseenter', function (ev) {
                                return _this.trigger('eventRender', mySegEl, myEvent);
                            });
                        })(this, e, segEl);
...
var friHours = 0;
var satHours = 0;
			 $( ".fc-sun" ).each(function( index ) {
                	  
                	  var sun = $( this );
					  var mon = sun.closest('tr').find('.fc-mon').text();
					   var tue = sun.closest('tr').find('.fc-tue').text();
					   var wed = sun.closest('tr').find('.fc-wed').text();
					   var thu = sun.closest('tr').find('.fc-thu').text();
					   var fri = sun.closest('tr').find('.fc-fri').text();
					   var sat = sun.closest('tr').find('.fc-sat').text();
                	  sunHours += Number(sun.text());
					   monHours += Number(mon);
					   tueHours += Number(tue);
					   wedHours += Number(wed);
					   thuHours += Number(thu);
             ...
             this.el.find(".fc-footer.fc-sun").html(sunHours);
			 this.el.find(".fc-footer.fc-mon").html(monHours);
			  this.el.find(".fc-footer.fc-tue").html(tueHours);
			  this.el.find(".fc-footer.fc-wed").html(wedHours);
			   this.el.find(".fc-footer.fc-thu").html(thuHours);
			   this.el.find(".fc-footer.fc-fri").html(friHours);
...
//start myBasicView
var BasicView = fcViews.basicDay = View.extend({
...
var currentDayStart = this.start.clone();
          ...
          var header =$('\
        		  <div class="box-header with-border" style="border-bottom:2px solid #DDDDDD"><div class="pull-left ">\
                  <button  class="btn btn-success btn-sidebar" id="newTimeEntryButton" data-toggle="modal" data-target="#addWeeklyReturnModal">\
    						<i class="fa fa-calendar-plus-o"></i>&nbsp; Add Time Entry</button>\
                <button  class="btn btn-danger btn-sidebar" id="deleteTimeEntryButton">\
    						<i class="glyphicon glyphicon-trash"></i>&nbsp; Delete selected</button>\
        		 </div>\
        		</div>');
			var tbody= $('\
			<table class="table table-striped">\
				...
				<tr>\
					<th class="col-md-3" ></th>\
					<th class="col-md-1" id="totalHours"></th>\
					<th class="col-md-1"></th>\
				</tr>\
				</tfoot>\
				<tbody></tbody>\
                ...
                for (var i = eventsCopy.length - 1; i >= 0; --i) {
                    ...
                    if (currentDayStart.isAfter(eventEnd) || (currentDayStart.isSame(eventEnd) && !eventStart.isSame(eventEnd)) || periodEnd.isBefore(eventStart)) {
                       ...
                       var segEl = $('\
							<tr>\
                    		   <td  class="col-md-3"><input type="checkbox" id="timesheetsCheckbox" value="'+e+'">&nbsp;<small>['+e.project.id+']</small>&nbsp;<strong>'+e.project.projectName+'</strong><br \><span>'+e.activity.activityName+'<br \><span>'+'</span></td>\
								<td class="col-md-1 text-center" ><span class="info-box-number">'+e.hours+'</span></td>\
								<td class="col-md-1 text-right">\
								<button id="editWeeklyReturn" data-toggle="modal" data-title="Edit" class="btn btn-primary btn-xs"><span class="glyphicon glyphicon-pencil"></span></button>\
								<button class="btn btn-danger btn-xs" id="trashWeeklyReturn" data-toggle="modal" data-target="#deleteWeeklyReturnModal"><span class="glyphicon glyphicon-trash"></span></button>\
                            </td>\
							</tr>');
							var currentDay = '.fc-'+currentDayStart.format('ddd').toLowerCase();
							segEl.find(''+currentDay+'').html(e.hours);
 ...
 if(eventsCopy.length > 0){
                ...
                $( ".info-box-number" ).each(function( index ) {
                	  ...
                	  var eventPrice = $( this ).text();
                	  totalHours += Number(eventPrice);
                	});
					 var totalWorked= $('\<span> total : '+totalHours+'</span>');
					this.el.find("#totalHours").html(totalWorked);
 }else{
